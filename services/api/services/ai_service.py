...
Your task: produce exactly ONE JSON object that conforms to Blueprint schema version 1.0 through 4.3.

Hard rules:
- Root keys: required metadata, global_settings, geometry, simulation. Optional global_variables (Blueprint v2.0 parametric constants). Optional assembly_mates (Blueprint v3.0+). Optional generators (Blueprint v4.3+). No other extra keys.
- metadata.schema_version: "1.0" … "4.3" (use "4.3" when you use generators.gearbox for a single-stage spur reducer; use "3.5" when you use constraint mates concentric/coincident/distance; use "3.2" when you include bearings or gears without v3.5 mates; use "3.0" when you only use snap_to_operation mates; "2.1" when you include fasteners without mates; "2.0" when you use global_variables and $expressions; "1.4" for revolved_profile without globals; "1.3" for extruded_profile; "1.2" for hole patterns; "1.1" for material presets on parts).
...
- **Gear reducers (MANDATORY when user asks for a reducer / gear pair / «сделай редуктор» / «собери передачу»):** Use schema **4.3** root array **"generators"** with one object: `type: "gearbox"`, numeric or `$global` fields `ratio` (1.5–10), `module`, `thickness`, `bore_diameter`, optional `center_distance` default `"auto"`, optional `high_lod` boolean. Put tunable values in **global_variables** (e.g. `gearbox_ratio`, `gearbox_module`) and reference them from the generator with `$name`. Use **geometry.parts: []** (empty) when the generator is the sole source of parts — the server expands to two gears, two shafts, mates, and derived globals before CAD. **Do NOT** hand-place two gears in `geometry.parts` and hand-compute center distance for this use case.
- Gear reducers (manual two+ meshing spur gears, same module, when user explicitly wants custom layout without generators): Put shaft/hole spacing on a single source of truth in global_variables. Define e.g. `center_distance` = half-sum of pitch diameters plus backlash so the resolver computes it: `center_distance = (module * teeth1 + module * teeth2) / 2 + (0.05 * module)` (module must be identical for both gears). Use string expressions with `$center_distance` (and `$module`, `$teeth1`, `$teeth2` if helpful) for positions of parallel shaft holes in the housing — distance between gear rotation axes must equal `center_distance` exactly. With schema 3.5 you may instead drive spacing via assembly_mates type "distance" and value "$center_distance" after concentric mates to the holes. Do not hand-type unrelated decimals for meshing pairs.
...
EDIT_SYSTEM_PROMPT = """You are editing an existing AI-Forge Blueprint v1.0/v1.1/v1.2/v1.3/v1.4/v2.0/v2.1/v3.0/v3.2/v3.5/v4.3 (mechanical CAD).
...
Your task:
- Return exactly ONE complete JSON object that still conforms to Blueprint schema v1.0 through v4.3.
...
def _repair_prompt(invalid_data: dict[str, Any], err: ValidationError) -> str:
    return (
        "The previous JSON failed Pydantic validation for Blueprint (v1.x through v4.3). "
