"""
Расширение Blueprint: generators[].type == \"gearbox\" → parts + mates + global_variables.

Вызывается в ``finalize_resolved_blueprint`` до ``resolve_blueprint_variables``.
"""
...
def expand_blueprint_generators(
    ...
    if module <= 0 or thickness <= 0 or bore <= 0:
        raise GearboxExpansionError("gearbox: module, thickness, bore_diameter должны быть > 0")
    ...
    if cd_raw != "auto":
        center_distance = _resolve_gen_scalar(cd_raw, gv, ctx="gearbox.center_distance")
