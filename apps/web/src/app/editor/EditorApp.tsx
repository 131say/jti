...
function EditorAppInner() {
  ...
  const handleFork = useCallback(async () => {
    ...
    try {
      ...
      loadedIdRef.current = null;
      router.replace(`/editor?project=${encodeURIComponent(r.project_id)}`, {
        scroll: false,
      });
  ...
  const handleSaveProject = useCallback(async () => {
    if (isLiveDemo) {
      setToastMsg(
        "Демо без облака: войдите и создайте свой проект в редакторе, чтобы сохранить.",
      );
      return;
    }
    if (!user) {
  ...
  const handleShareProject = useCallback(() => {
    if (!projectIdParam) return;
    const url = `${window.location.origin}/editor?project=${encodeURIComponent(projectIdParam)}`;
  ...
  const onRunCode = () => {
    if (workspaceReadOnly && !isLiveDemo) {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      {toastMsg ? (
        <div
          className={`fixed bottom-4 left-1/2 max-w-md -translate-x-1/2 rounded border border-amber-800 bg-amber-950 px-4 py-2 text-center text-xs text-amber-100 shadow-lg md:left-auto md:right-4 md:translate-x-0 ${
            isLiveDemo ? "z-[110]" : "z-50"
          }`}
          role="alert"
        >
          {toastMsg}
